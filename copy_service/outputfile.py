import numpy as np
import pandas as pd
def outputfile(sf_oracle, project_cost,total_cost ,tag):
    try:
        Tag = tag  ## Has to be Onshore/Shared/Offshore
        ...
        dfl = pd.read_excel(project_cost, None)
        dfl1 = []
        type1 = []
        ...
        conv = {'Account': str, 'Branch': str, 'Cost Centre': str, 'Service Line': str, 'Program': str, 'Project': str,
                'Future1': str}
        dfl = pd.read_excel(total_cost, None, converters=conv)
        dfl1 = []
        type2 = []
        for type_, df in dfl.items():
            type2.append(type_)
        ...
        # Dec
        oracle = pd.read_excel('copy_service/output_v1.xlsx')
        oracle.head(1)
